    ...
    <div className="flex flex-col min-h-screen bg-white">
      <header className="sticky-header bg-white/95 backdrop-blur-md px-4 py-4 border-b border-[#e8e8e8]">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-[#1a1a1a] font-song">情绪惯性模式</h1>
            <p className="text-base text-[#666666] mt-0.5">Sino-NLP × 四体模型：揭露你的情绪反应惯性</p>
          </div>
          <HomeLink />
        </div>
      </header>
      ...
      <div className="flex-1 px-4 pb-32 content-below-header">
        ...
        <div className="card">
          ...
          <ul className="space-y-2.5 text-base text-[#666666]">
            <li className="flex items-start gap-2">
              ...
              本测评基于 Sino-NLP 方法论，结合四体模型
            </li>
            <li className="flex items-start gap-2">
              <span className="text-[#c9a84c] font-bold mt-0.5">•</span>
              共 {TOTAL_EMOTION} 题，预计需时 5-8 分钟
            </li>
        ...
        <div className="mt-4 card">
          <h3 className="text-lg font-semibold text-[#1a1a1a] mb-3">五大惯性维度</h3>
          <div className="space-y-2">
            {EMOTION_DIMENSIONS.map((dim, i) => (
...
// ─── 答题页 ───
function EmotionQuiz({ onComplete }: { onComplete: (answers: Record<number, number>) => void }) {
  ...
  const progress = Math.round((answeredCount / TOTAL_EMOTION) * 100);
  const allAnswered = answeredCount === TOTAL_EMOTION;
  const currentSelected = answers[question.id];

  const currentDimLabel = question.dimensionLabel;
  const dimQuestions = EMOTION_QUESTIONS.filter((q) => q.dimension === question.dimension);
    ...
    <div className="flex flex-col min-h-screen bg-white quiz-protected">
      <nav className="sticky-header z-30 border-b border-[#e8e8e8] bg-white/95 backdrop-blur-md px-4 py-3">
        <div className="flex items-center gap-2 mb-2">
          <HomeLink />
          <Sparkles size={20} className="text-[#c9a84c]" />
          <span className="text-base font-semibold text-[#1a1a1a]">情绪惯性模式</span>
          <span className="ml-auto text-base text-[#666666]">
            {answeredCount}/{TOTAL_EMOTION}
          </span>
        </div>
      ...
      <section className="flex-1 px-4 pt-2 pb-40 content-below-quiz-nav">
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            onClick={() => {
              setCurrentQ((p) => Math.min(TOTAL_EMOTION - 1, p + 1));
              setShowTip(false);
            }}
            disabled={currentQ === TOTAL_EMOTION - 1}
            className="flex-1 py-3 rounded-xl border border-[#e8e8e8] text-base text-[#666] disabled:opacity-30 disabled:cursor-not-allowed"
          >
            下一题
          </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-[#e8e8e8] bg-white/95 backdrop-blur-md px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))]">
        {!allAnswered && (
          <p className="mb-2 text-center text-base text-[#666666]">
            已完成 {answeredCount}/{TOTAL_EMOTION} 题
          </p>
        )}
        <button
    ...
    <div className="flex flex-col min-h-screen bg-white">
      <nav className="sticky-header z-30 border-b border-[#e8e8e8] bg-white/95 backdrop-blur-md px-4 py-3">
        <div className="flex items-center gap-2">
          <HomeLink />
          <Sparkles size={20} className="text-[#c9a84c]" />
          <span className="text-base font-semibold text-[#1a1a1a]">情绪惯性模式结果</span>
        </div>
      </nav>
      ...
      <div className="flex-1 px-4 pb-20 content-below-quiz-nav">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-[#1a1a1a] font-song">你的情绪惯性等级</h1>
        ...
        <div className="text-center pt-2 pb-2">
          ...
          <p className="text-xs text-[#999999]">Sino-NLP 中华身心语言学 · 四体模型</p>
        </div>
...
// ─── 主组件 ───
export default function EmotionInertiaPage({ onBack }: { onBack?: () => void }) {
  ...
  const handleComplete = useCallback((ans: Record<number, number>) => {
    setAnswers(ans);
    const res = calcEmotionResult(ans);
    setResult(res);
    setStage("result");
  }, []);
