    ...
    <div className="flex flex-col min-h-screen bg-white">
      <header className="sticky top-0 z-10 bg-white/95 backdrop-blur-md px-4 py-4 border-b border-[#e8e8e8]">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-xl font-bold text-[#1a1a1a] font-song">PCL-5 PTSD 检查表</h1>
            <p className="text-sm text-[#666666] mt-0.5">PTSD Checklist for DSM-5</p>
          </div>
        </div>
      </header>
    ...
    <div className="flex flex-col min-h-screen bg-white">
      <nav className="sticky top-0 z-30 border-b border-[#e8e8e8] bg-white/95 backdrop-blur-md px-4 py-3">
        <div className="flex items-center gap-2 mb-2">
          <Sparkles size={18} className="text-[#c9a84c]" />
          <span className="text-sm font-semibold text-[#1a1a1a]">PCL-5 创伤筛查</span>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      <nav className="sticky top-0 z-30 border-b border-[#e8e8e8] bg-white/95 backdrop-blur-md px-4 py-3">
        <div className="flex items-center gap-2">
          <Sparkles size={18} className="text-[#c9a84c]" />
          <span className="text-sm font-semibold text-[#1a1a1a]">PCL-5 评估结果</span>
      ...
      <div className="flex-1 px-4 pt-4 pb-20">
        <div ref={reportRef} className="bg-white">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-[#8888aa] mt-1">/ 80</p>
            <div
          ...
          <div className="space-y-1 mb-4">
            ...
            {PCL5_QUESTIONS.map((q) => {
              const score = answers[q.id] ?? 0;
              const label = PCL5_OPTIONS.find((o) => o.score === score)?.label ?? "";
              return (
        ...
        <div className="mt-4 rounded-xl border border-[#c9a84c30] bg-gradient-to-br from-[#fdf8ed] to-white p-5">
          <div className="text-center mb-3">
            ...
            <h3 className="text-lg font-bold text-[#1a1a1a] font-song">获取专属创伤疗愈方案</h3>
          </div>
...
export default function PCL5AssessmentPage({ onBack }: { onBack?: () => void }) {
  ...
  return (
    <>
      ...
      {stage === "result" && result && <PCL5ResultView result={result} answers={answers} onRestart={handleRestart} />}
    </>
  );
}
