...
function GAD7Quiz({ onComplete }: { onComplete: (answers: Record<number, number>) => void }) {
  ...
  const allAnswered = answeredCount === TOTAL_GAD7;
...
function GAD7ResultView({
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      link.download = `GAD7焦虑报告_${testDate.replace(/\//g, "-")}.png`;
      link.href = dataUrl;
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      ...
      if (navigator.share) {
        ...
        const file = new File([blob], "GAD7焦虑报告.png", { type: "image/png" });
        await navigator.share({ title: "我的焦虑评估报告 - 亦须AI", text: "测测你的焦虑水平 👇", files: [file] });
      }
    } catch (err) {
...
// ─── 主组件 ───
export default function GAD7AssessmentPage({ onBack }: { onBack?: () => void }) {
