...
/** 將數字逐位相加（字符串版，如 "20260609" → 2+0+2+6+0+6+0+9=25） */
...
export interface DivinationResult {
  ...
  season?: Season;        // 季節（入口一專用）
  topic?: string;         // 諮詢課題（入口二專用）
  score?: number;         // 狀態分（入口二專用）
}
...
export interface YaoReport {
  position: number;       // 1-6
  isDongYao: boolean;     // 是否動爻
  yaoText: string;       // 爻辭原文
  level: string;         // 理解層次名
  isYang: boolean;       // 是否陽爻
  lineSymbol: string;    // 爻符號（⚊ / ⚋）
}
