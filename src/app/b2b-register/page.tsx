...
export default function B2BRegisterPage() {
  ...
  if (result) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white px-6">
        ...
        <h1 className="text-xl font-bold text-[#1a1a1a] mb-2">註册成功！</h1>
        <p className="text-sm text-[#666] text-center mb-6">
          你已獲得 <span className="font-bold text-[#c9a84c]">7 日免费试用</span><br />
          试用期内可无限使用所有测评功能
        </p>
        <div className="card w-full max-w-xs text-left space-y-2 mb-6">
          ...
          <p className="text-sm"><span className="text-[#999]">试用到期：</span>{new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString("zh-HK")}</p>
        </div>
        <p className="text-xs text-[#999] mb-6">
          试用期结束后，请加微信 <span className="font-bold text-[#c9a84c]">859022196</span> 續费
        </p>
        <button
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white px-6">
      ...
      <h1 className="text-xl font-bold text-[#1a1a1a] mb-1">B2B 合作註册</h1>
      <p className="text-sm text-[#666] text-center mb-8">
        註册即享 7 日免费试用，体验全部测评功能
      </p>

      <div className="w-full max-w-xs space-y-4">
        <div>
          <label className="text-sm text-[#666] mb-1 block">公司/机构名稱</label>
          <input
            type="text"
            value={companyName}
            onChange={(e) => setCompanyName(e.target.value)}
            placeholder="例如：静心心理咨询中心"
            className="w-full px-4 py-3 rounded-xl border border-[#e8e8e8] outline-none focus:border-[#c9a84c]"
          />
        </div>
        <div>
          <label className="text-sm text-[#666] mb-1 block">微信号</label>
          <input
            type="text"
            value={wechatId}
