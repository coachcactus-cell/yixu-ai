...
const ADMIN_PASSWORD = "yixu2024"; // 簡单密码保护
...
export default function B2BAdminPage() {
  ...
  const fmtStatus = (client: B2BClient): { label: string; color: string } => {
    ...
    if (client.plan === "trial" && Date.now() > client.trialEnd) return { label: "试用过期", color: "text-orange-500" };
    if (client.plan === "trial") return { label: "试用中", color: "text-blue-500" };
    if (client.subscriptionEnd > 0 && Date.now() > client.subscriptionEnd) return { label: "订阅过期", color: "text-orange-500" };
    return { label: "付费中", color: "text-green-500" };
  };
  ...
  if (!authenticated) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white px-6">
        ...
        <h1 className="text-xl font-bold text-[#1a1a1a] mb-2">B2B 后台管理</h1>
        <p className="text-sm text-[#999] mb-6">请输入管理密码</p>
        <input
          type="password"
          value={password}
      ...
      <header className="sticky top-0 z-10 bg-white/95 backdrop-blur-md px-4 py-3 border-b border-[#e8e8e8]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-[#1a1a1a]">B2B 后台</h1>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-8">
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <div className="card text-center">
            ...
            <p className="text-xs text-[#999]">总客戶</p>
          </div>
          <div className="card text-center">
            ...
            <p className="text-xs text-[#999]">试用中</p>
          </div>
          <div className="card text-center">
            ...
            <p className="text-xs text-[#999]">付费</p>
          </div>
        </div>
        ...
        {clients.length === 0 && (
          <p className="text-sm text-[#999] text-center py-8">暫无客戶</p>
        )}
            ...
            return (
              <div key={client.id} className="card">
                ...
                <div className="text-xs text-[#999] space-y-1 mb-3">
                  <p>试用期: {fmtDate(client.trialStart)} ~ {fmtDate(client.trialEnd)}</p>
                  {client.subscriptionEnd > 0 && (
                    <p>订阅到期: {fmtDate(client.subscriptionEnd)}</p>
                  )}
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <select
                    ...
                    defaultValue=""
                  >
                    <option value="">+ 开通功能</option>
                    <option value="enneagram">九型人格</option>
                    <option value="chakra">7脉轮</option>
        ...
        {stats && selectedClient && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => { setStats(null); setSelectedClient(null); }}>
            <div className="bg-white rounded-2xl p-6 m-4 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="text-xs text-[#999] mb-4">测评统计</p>
              <div className="space-y-2 text-sm">
                <p>总测评次数: <span className="font-bold">{stats.totalAssessments}</span></p>
                <p>九型人格: {stats.byType.enneagram} 次</p>
                <p>7脉轮: {stats.byType.chakra} 次</p>
                <p>易卦: {stats.byType.yijing} 次</p>
                <p>最后活动: {stats.lastActivity ? fmtDate(stats.lastActivity) : "—"}</p>
              </div>
              <button
