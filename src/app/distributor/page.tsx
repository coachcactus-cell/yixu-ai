...
const MOCK_CUSTOMERS = [
  { id: "c1", name: "小明", signupDate: "2026-05-15", source: "推廣连结", assessments: 3, vip: false },
  { id: "c2", name: "阿静", signupDate: "2026-05-18", source: "二維码", assessments: 5, vip: true },
  { id: "c3", name: "修行者A", signupDate: "2026-05-22", source: "推廣连结", assessments: 2, vip: false },
  { id: "c4", name: "瑜伽同学", signupDate: "2026-06-01", source: "二維码", assessments: 1, vip: false },
  { id: "c5", name: "芳疗爱好者", signupDate: "2026-06-08", source: "推廣连结", assessments: 4, vip: true },
];
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/50">分銷商后台 · {MOCK_DIST.referralCode}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-3 mt-4">
        <KPICard label="点擊量" value={s.totalClicks} change={s.clicksChange} icon={Eye} color="#c9a84c" />
        <KPICard label="註册数" value={s.totalSignups} change={s.signupChange} icon={Users} color="#6366f1" />
        <KPICard label="转化率" value={s.conversionRate} change={s.convChange} icon={TrendingUp} color="#8b5cf6" suffix="%" />
        <KPICard label="待结佣金" value={s.pendingCommission} change={s.commChange} icon={DollarSign} color="#10b981" prefix="¥" />
      </div>
      ...
      <div className="mt-4 bg-white rounded-xl p-4 border border-[#e8e8e8]">
        ...
        <div className="flex items-center gap-2 bg-[#fafafa] rounded-lg p-3">
          ...
          <button className="text-xs text-[#c9a84c] font-bold flex-shrink-0">复制</button>
        </div>
      </div>
    </>
  );
    ...
    <>
      <div className="mt-4 bg-white rounded-xl p-4 border border-[#e8e8e8]">
        ...
        <div className="bg-[#fafafa] rounded-lg p-4 mb-3">
          <p className="text-xs text-[#999] mb-1">你的专屬推廣连结</p>
        ...
        <div className="flex flex-col items-center py-4">
          <div className="w-40 h-40 bg-[#fafafa] rounded-xl border border-[#e8e8e8] flex items-center justify-center mb-3">
            <div className="text-center">
              ...
              <p className="text-[9px] text-[#999]">掃码进入亦须AI</p>
            </div>
          </div>
      ...
      <div className="mt-4 bg-white rounded-xl p-4 border border-[#e8e8e8]">
        ...
        <div className="space-y-2">
          {[
            { name: "亦须AI 介紹海报", type: "图片", size: "2.4 MB" },
            { name: "九型人格测评推廣文", type: "文案", size: "1.2 KB" },
            { name: "七脉轮体验分享模板", type: "文案", size: "0.8 KB" },
          ].map((item) => (
    ...
    <>
      <div className="mt-4 grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl p-3 border border-[#e8e8e8] text-center">
          ...
          <p className="text-[10px] text-[#999]">总客戶</p>
        </div>
        <div className="bg-white rounded-xl p-3 border border-[#e8e8e8] text-center">
        ...
        <div className="bg-white rounded-xl p-3 border border-[#e8e8e8] text-center">
          ...
          <p className="text-[10px] text-[#999]">测评数</p>
        </div>
      </div>
        ...
        {MOCK_CUSTOMERS.map((c) => (
          <div key={c.id} className="bg-white rounded-xl p-3 border border-[#e8e8e8] flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-[#999]">来源: {c.source} · {c.signupDate}</p>
            </div>
            <div className="text-right flex-shrink-0">
              <p className="text-xs text-[#666]">{c.assessments} 测评</p>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <>
      <div className="mt-4 bg-gradient-to-br from-[#1a3a2e] to-[#1a2e3a] rounded-xl p-5 text-white">
        <p className="text-xs text-white/60 mb-1">可提现佣金</p>
        <p className="text-3xl font-bold">¥{pending}</p>
        <p className="text-xs text-white/50 mt-1">累计收入: ¥{totalEarned}</p>
      </div>

      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="bg-white rounded-xl p-3 border border-[#e8e8e8]">
          <p className="text-xs text-[#999]">已提现</p>
      ...
      <div className="mt-4 bg-white rounded-xl p-4 border border-[#e8e8e8]">
        <h3 className="text-sm font-bold text-[#1a1a1a] mb-3">收入明细</h3>
